...
1. 真值：符号用“+“、“-”表示的数据表示方法。
2. 机器数：符号数值化的数据表示方法，用0、1表示符号。（原码、反码、补码）（以下不详细介绍定义，仅讨论性质，如有需要则参考CSAPP第二章）。
...
1. 表示简单
2. 运算复杂：符号位不参与运算，需要单独设计减法器
3. 0的表示不唯一

#### 反码

1. 表示相对原码更复杂
2. 运算相对原码更简单：符号位参与运算（符号位进位要加到最低位），无需设计减法器。
3. 0的表示不唯一
...
移码用来表示浮点数的阶码，如IEEE754中的阶码。

具体实现：数值位与真值的补码相同，符号位相反。
...
- IEEE754格式：见CSAPP第二章笔记，这里给个单精度的表示简述：
